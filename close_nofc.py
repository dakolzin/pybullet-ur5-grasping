...
class HandParams:
    ...
    roi_margin: float = 0.015  # 1 см запас
    y_push_max: float = 0.03 
...
def darboux_frame_from_quadric(p0: np.ndarray, neigh: np.ndarray) -> Optional[np.ndarray]:
    X = (neigh - p0[None, :]).astype(np.float64)
    v = quadric_fit_taubin_like(X)
    if v is None:
        return None

    a, b, c, d, e, f, g, h, i, _ = [float(x) for x in v.tolist()]
    ...
    return np.column_stack([t, bvec, n])


def estimate_darboux_frame(p0: np.ndarray, neigh: np.ndarray, method: str) -> Optional[np.ndarray]:
    if method == "pca":
        return make_darboux_frame_from_pca(neigh)
    if method == "taubin":
        R = darboux_frame_from_quadric(p0, neigh)
        if R is not None:
            return R
        return make_darboux_frame_from_pca(neigh)
    ...
    for phi in phis:
        ...
        for w in widths:
            ...
            if best_center is not None:
                # score: без FC — просто предпочитаем меньше push и разумную ширину
                score = - (float(best_push)) - 1.2 * w
...
def main():
    ...
    ap.add_argument("--no_above", action="store_true")
    ap.add_argument("--tilt", type=float, default=75.0)
    ...
    ap.add_argument("--zclr", type=float, default=0.008)
    ap.add_argument("--roi_margin", type=float, default=0.01)
    ...
    if len(hyps) > 0:
        h0 = hyps[0]
        print("Top score:", h0.score, "width:", h0.width, "push:", h0.push, "center:", h0.center)
